...
public enum SceneRendererMode {
    ...
    private static final boolean cVarSerial = SystemHandler.instance.getCvars().bool(CommandVariable.SERIALRENDERER);
    private static final boolean cVarParallel = SystemHandler.instance.getCvars().present(CommandVariable.PARALLELRENDERER);
    private static final boolean cVarParallel2 = SystemHandler.instance.getCvars().present(CommandVariable.PARALLELRENDERER2);
    ...
    static int[] parseSwitchConfig(CommandVariable sw) {
        ...
        final int walls = SystemHandler.instance.getCvars().get(sw, Integer.class, 0).orElse(1);
        // Try parsing floors. If wall succeeded, but floors not, it will default to 1.
        final int floors = SystemHandler.instance.getCvars().get(sw, Integer.class, 1).orElse(1);
        // In the worst case, we will use the defaults.
        final int masked = SystemHandler.instance.getCvars().get(sw, Integer.class, 2).orElse(2);
    ...
    static SceneRendererMode getMode() {
         ...
         */
        return SystemHandler.instance.getConfig().getValue(Settings.scene_renderer_mode, SceneRendererMode.class);
    }
